...
*/
    ...
    setup_goal_pose_target(+0.340, -0.020, +0.220, -1.000, +0.000, +0.000, +0.000);
    plan_trajectory_kinematics();
    execute_trajectory_kinematics();
    ...
    // 4. Cartesian Approach
    RCLCPP_INFO(LOGGER, "Approaching (Cartesian Line)...");
    move_group_robot_->setMaxVelocityScalingFactor(0.05);
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    execute_cartesian_trajectory(+0.340, -0.020, (target_pose_robot_.position.z - delta_), -1.000, +0.000, +0.000, +0.000);

    // 5. Close the gripper
    RCLCPP_INFO(LOGGER, "Closing Gripper...");
    move_group_robot_->setMaxVelocityScalingFactor(0.05);
    setup_joint_value_gripper(+0.646);
    plan_trajectory_gripper();
    execute_trajectory_gripper();
...
private:
  ...
  // Kept at 5cm based on previous collision avoidance math
  float delta_ = 0.045; // meters
  ...
  void add_collision_objects() {
    ...
    RCLCPP_INFO(LOGGER, "Adding Table to Planning Scene...");
  ...
  void execute_cartesian_trajectory(float pos_x, float pos_y, float pos_z,
    ...
    // Push the singular target pose to force a straight line from current state
    ...
    RCLCPP_INFO(LOGGER, "Cartesian path calculated: %.2f%% achieved", fraction * 100.0);

    // Execute if a large portion of the path was viable
    if (fraction >= 0.9) {
      move_group_robot_->execute(trajectory);
      RCLCPP_INFO(LOGGER, "Cartesian Trajectory Executed Successfully!");
    } else {
      RCLCPP_ERROR(LOGGER, "Cartesian Trajectory Failed! Path obstructed or out of reach.");
    }
  }
